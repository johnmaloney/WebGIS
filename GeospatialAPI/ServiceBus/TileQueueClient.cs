...
{
    public class TileQueueClient : ITileQueueClient
    {
        #region Fields

        private readonly QueueClient tileResponseClient;
        private readonly QueueClient tileResultClient;
        ...
        public TileQueueClient(QueueClientOptions options)
        {
            tileResponseClient = new QueueClient(options.ConnectionString, ServiceBusKeys.TileQuery);

            if (options.AppType == AppKeys.Responder)
            {
                tileResultClient = new QueueClient(options.ConnectionString, ServiceBusKeys.TileResult);
        ...
        public async Task SendMessages(TileMessage tile)
        {
            try
            {
                ...
                await tileResponseClient.SendAsync(message);
            }
        ...
        public void RegisterOnMessageHandlerAndReceiveMessages()
        {
            ...
            tileResponseClient.RegisterMessageHandler(ProcessMessagesAsync, messageHandlerOptions);
        }
        ...
        public async Task ProcessMessagesAsync(Message message, CancellationToken token)
        {
            ...
            await tileResponseClient.CompleteAsync(message.SystemProperties.LockToken);

            // Note: Use the cancellationToken passed as necessary to determine if the queueClient has already been closed.
            // If queueClient has already been Closed, you may chose to not call CompleteAsync() or AbandonAsync() etc. calls 
            // to avoid unnecessary exceptions.

            Task.Delay(tileMessage.LatencyTime).Wait();

            string messageBody = JsonConvert.SerializeObject(new TileMessage { ClientName = tileMessage.ClientName, SessionId = tileMessage.SessionId, Result = this.sampleGeoJSON() } );
            ...
            await tileResultClient.SendAsync(resultMessage);
        }
